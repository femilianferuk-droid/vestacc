...
.navbar{background:rgba(8,8,18,0.92);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom:1px solid rgba(255,255,255,0.06);padding:12px 20px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:1010}
...
.burger{width:34px;height:34px;background:transparent;border:none;cursor:pointer;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:4px;position:relative;border-radius:6px;transition:0.2s;z-index:1020}
.burger:hover{background:rgba(255,255,255,0.05)}
.burger span{display:block;width:20px;height:2px;background:#e2e8f0;border-radius:2px;transition:0.3s ease}
.burger.open span:nth-child(1){transform:translateY(6px) rotate(45deg)}
.burger.open span:nth-child(2){opacity:0}
.burger.open span:nth-child(3){transform:translateY(-6px) rotate(-45deg)}

.sidebar{position:fixed;top:0;right:0;width:260px;height:100vh;background:#0a0a14;border-left:1px solid rgba(255,255,255,0.06);z-index:1000;transition:transform 0.3s ease;padding:80px 16px 16px;display:flex;flex-direction:column;gap:6px;transform:translateX(100%)}
...
.modal{position:fixed;top:0;left:0;right:0;bottom:0;z-index:1200;display:none;align-items:center;justify-content:center}
...
SCRIPT = '''
<script>
// Глобальный перехватчик событий клика по всей странице
document.body.addEventListener('click', function(e) {
    
    // 1. Кнопка меню 3 полоски (Бургер)
    ...
    // 2. Затемненный фон меню (Overlay)
    ...
    // 3. Переключатель панели фильтров
    if (e.target.closest('.filter-btn')) {
        document.getElementById('filterDrop').classList.toggle('show');
        return;
    }

    // 4. Открытие выпадающих списков Мультиселекта
    ...
    // 5. Открытие модальных окон (Вывод / Продажа)
    ...
    if (copyBtn) {
        ...
        navigator.clipboard.writeText(text).then(function() {
            ...
            el.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;padding:10px 20px;border-radius:30px;font-weight:600;z-index:9999;box-shadow:0 8px 25px rgba(99,102,241,0.5)';
    ...
    // 8. Получить код авторизации Telegram
    ...
    // Закрытие мультиселектов при клике в любое другое место
...
// Живой поиск внутри мультиселектов
...
// Реактивный выбор элементов мультиселекта
...
def withdraw_modal():
    ...
    return f'<div class="modal" id="withdrawModal"><div class="modal-bg btn-close-modal" data-modal="withdrawModal"></div><div class="modal-content"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 style="font-size:18px;font-weight:700">Вывод средств</h3><button class="btn-close-modal" data-modal="withdrawModal" style="background:transparent;border:none;color:#94a3b8;font-size:22px;cursor:pointer">&times;</button></div><p style="color:#64748b;margin-bottom:12px;font-size:13px">Баланс: <strong style="color:#34d399">{g.user["balance"]:.2f} ₽</strong> | Продаж: <strong style="color:#a5b4fc">{g.user.get("sales_count",0)}</strong></p>{c}</div></div>'
    ...
    if request.method == 'POST':
        ...
        if not title or not price: flash('Название и цена обязательны', 'error')
            ...
            try:
                ...
                flash('Acc выставлен!', 'success'); return redirect(url_for('index'))
            except Exception as e: flash(f'Ошибка: {e}', 'error')
