...
.sidebar .sidebar-divider{height:1px;background:rgba(255,255,255,0.06);margin:6px 0}
...
.profile-actions-row .action-btn:hover {
    background: #1e1e2f;
    border-color: rgba(99, 102, 241, 0.2);
    color: #fff;
}

.profile-actions-row .action-btn.primary {
    background: rgba(99, 102, 241, 0.1);
    border: 1px solid rgba(99, 102, 241, 0.2);
    color: #a5b4fc;
}

.profile-actions-row .action-btn.success {
    background: rgba(16, 185, 129, 0.1);
    border: 1px solid rgba(16, 185, 129, 0.2);
    color: #34d399;
}

.profile-actions-row .action-btn.warning {
    background: rgba(245, 158, 11, 0.1);
    border: 1px solid rgba(245, 158, 11, 0.2);
    color: #fbbf24;
}

.profile-actions-row .action-btn.danger {
...
def navbar():
    if g.user:
        admin_divider = '<div class="sidebar-divider"></div><a href="/admin" style="color:#a5b4fc;">👑 Админ панель</a>' if g.user.get("is_admin") else ''
        return f'''
        <div class="navbar">
...
def my_purchases():
    try:
        db = get_db()
        with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT p.id, p.buyer_id, p.account_id, p.phone_number, p.purchase_date, p.code_retrieved, a.title, a.session_string FROM purchases p JOIN accounts a ON p.account_id = a.id WHERE p.buyer_id = %s ORDER BY p.id DESC", (g.user['id'],))
...
def profile():
    ...
    <div class="profile-container">
        ...
        <div class="profile-actions-row">
            <a href="/deposit" class="action-btn success">💰 Пополнение</a>
            <button class="action-btn warning window-trigger-btn" data-window="win-withdraw">💸 Вывод</button>
            <button class="action-btn primary window-trigger-btn" data-window="win-sell">📦 Продать</button>
        </div>

        <div class="profile-actions-row">
            <a href="/my_purchases" class="action-btn">🛒 Мои покупки</a>
            <a href="/logout" class="action-btn danger">🚪 Выйти</a>
        </div>
