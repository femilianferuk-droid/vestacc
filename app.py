...
body{font-family:'Inter',sans-serif;background:#0d0d14;color:#f1f5f9;min-height:100vh;overflow-x:hidden}
...
.logo{font-size:24px;font-weight:900;text-decoration:none;letter-spacing:-0.8px;background:linear-gradient(90deg, #6366f1, #3b82f6, #8b5cf6, #6366f1);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:shimmer 3s linear infinite}
...
.sidebar a{display:flex;align-items:center;gap:12px;padding:14px;color:#cbd5e1;text-decoration:none;border-radius:12px;font-weight:600;transition:0.2s;font-size:14px;border:1px solid rgba(255,255,255,0.06);background:rgba(255,255,255,0.01)}
.sidebar a:hover{background:rgba(255,255,255,0.04);color:#fff;border-color:rgba(99,102,241,0.3)}
...
.bottom-sheet {position:fixed;bottom:0;left:0;right:0;background:#060610;border-top:1px solid rgba(255,255,255,0.06);border-radius:26px 24px 0 0;z-index:2010;transform:translateY(100%);transition:transform 0.3s ease;max-height:75vh;display:flex;flex-direction:column}
...
.custom-select-trigger {width:100%;padding:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);border-radius:14px;color:#cbd5e1;font-size:14px;cursor:pointer;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center}
...
.tag-yellow{background:rgba(245,158,11,0.08);color:#fbbf24}
.tag-blue{background:rgba(99,102,241,0.08);color:#a5b4fc}
.tag-red{background:rgba(239,68,68,0.08);color:#fca5a5}
...
def index():
    try:
        ...
        for a in accounts:
            ...
            card_stats = f'''
            <div class="stats">
                <div class="stat-tag">{a["country"] or "Неизвестно"}</div>
                <div class="stat-tag">{a["chats_count"]} Диалогов</div>
                <div class="stat-tag">{a["channels_count"]} Каналов</div>
                <div class="stat-tag">{"С 2Fa" if a["has_2fa"] else "Без 2Fa"}</div>
                <div class="stat-tag">{"Спамблок" if a["spamblock"] else "Без спамблока"}</div>
                <div class="stat-tag">{a["origin"] or "Лог"}</div>
            </div>
            '''
...
def account_detail(account_id):
    try:
        ...
        buy_btn = f'<form action="/buy/{a["id"]}" method="POST" style="flex:1;"><button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:12px;font-size:14px;">Купить</button></form>' if g.user and g.user['id'] != a['seller_id'] and not a['is_sold'] else ''
        ...
        <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px">
            <div>
                <h2 style="font-size:22px;font-weight:800; color:#fff;">{a["title"]}</h2>
