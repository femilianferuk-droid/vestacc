...
.bottom-sheet {position:fixed;bottom:0;left:0;right:0;background:#0d0d1f;border-top:1px solid rgba(255,255,255,0.08);border-radius:24px 24px 0 0;z-index:2010;transform:translateY(100%);transition:transform 0.35s cubic-bezier(0.4,0,0.2,1);max-height:75vh;display:flex;flex-direction:column;box-shadow:0 -10px 40px rgba(0,0,0,0.5)}
...
.sheet-item.selected {background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;border-color:transparent;box-shadow:0 4px 15px rgba(99,102,241,0.3)}

/* Измененный дизайн кастомного триггера */
.custom-select-trigger {width:100%;padding:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#cbd5e1;font-size:14px;font-weight:500;text-align:left;cursor:pointer;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;transition:0.2s}
.custom-select-trigger:hover {border-color:rgba(99,102,241,0.3);background:rgba(255,255,255,0.04)}
...
input,textarea{width:100%;padding:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#f1f5f9;font-size:14px;outline:none;transition:0.2s;font-family:inherit;margin-bottom:14px}
input:focus,textarea:focus{border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,0.15);background:rgba(255,255,255,0.04)}
...
SCRIPT = '''
<script>
// Глобальный перехватчик событий клика (Прямой обход CSP правил Telegram)
document.body.addEventListener('click', function(e) {
    ...
    // Открытие панели фильтров общего вида
    if (e.target.closest('.filter-btn')) {
        document.getElementById('filterDrop').classList.toggle('show');
        return;
    }

    // Открытие слайд-панелей (Bottom Sheets)
    ...
    // Клик по элементу внутри Bottom Sheet
    ...
    if (sheetItem) {
        ...
        if (sheetItem.parentElement.classList.contains('multi-select')) {
            // Множественный выбор (например, Страны)
            ...
            // Одиночный выбор (Премиум, Спамблок)
            ...
            // Авто-закрытие панели одиночного выбора
...
// Реализация встроенного "живого" поиска внутри Слайд-панели стран
...
def render_layout(title, content, show_nav=True):
    ...
    # Генерация Bottom Sheets разметки
    sheets = f'''
    ...
    <div class="bottom-sheet" id="sheet_country">
        ...
        <div class="bottom-sheet-content">
            <input type="text" class="sheet-search-input" placeholder="Поиск страны..." style="margin-bottom:14px;">
    ...
    <div class="bottom-sheet" id="sheet_origin">
        <div class="bottom-sheet-header"><h3>Происхождение аккаунта</h3><button class="bottom-sheet-close">&times;</button></div>
        <div class="bottom-sheet-content">
            <div class="sheet-grid multi-select">
    ...
    <div class="bottom-sheet" id="sheet_premium">
        <div class="bottom-sheet-header"><h3>Наличие Premium подписки</h3><button class="bottom-sheet-close">&times;</button></div>
        <div class="bottom-sheet-content">
    ...
    if request.method == 'POST':
        ...
        if amount < 20: flash('Минимум 20 ₽', 'error')
            ...
            try:
                ...
                data = {
                    "asset": "USDT", "amount": str(round(amount / 90, 2)), "description": "Пополнение Vest Accs",
    ...
    try:
        ...
        <div class="form-box" style="max-width:480px; margin: 40px auto;">
            ...
            <div style="background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.06); padding:20px; border-radius:14px; margin-bottom:20px;">
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;"><span style="color:#64748b;">Сумма к оплате:</span><strong style="color:#fff; font-size:16px;">{inv['amount_rub']:.2f} ₽</strong></div>
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;"><span style="color:#64748b;">Статус:</span><strong style="color:{status_color};">{status_text}</strong></div>
                <div style="display:flex; justify-content:space-between;"><span style="color:#64748b;">Дата:</span><span style="color:#94a3b8;">{inv['created_at'].strftime('%d.%m.%Y %H:%M')}</span></div>
            </div>
        '''
...
def profile():
    ...
    <div class="profile-section">
        ...
        <div class="profile-card">
            <h3 style="font-size:16px; font-weight:800; margin-bottom:12px; color:#fff;">Выставить аккаунт</h3>
            <form method="POST">
                ...
                <input type="text" name="phone" placeholder="+79001234567" required style="background: rgba(0,0,0,0.25);">
...
def verify_code_page():
    ...
    if request.method == 'POST':
        ...
        if action == 'confirm_code':
            ...
            try:
                # Восстанавливаем ОРИГИНАЛЬНЫЙ слепок сессии для предотвращения Code Expired
                ...
                try:
                    ...
                    flash('Введите пароль двухэтапной защиты (2FA).', 'info')
