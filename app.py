...
def init_db():
    ...
    with db.cursor() as cur:
        ...
        # Миграции структуры для старых баз данных (Защита от сбоев)
        ...
        # Проверка и создание указанного аккаунта администратора vest
        cur.execute("SELECT COUNT(*) FROM users WHERE username = %s", ("vest",))
        if cur.fetchone()[0] == 0:
            cur.execute("INSERT INTO users (username, password_hash, is_admin, balance, sales_count) VALUES (%s, %s, TRUE, 999999.00, 0)", ("vest", hash_password("vest55337q")))
    db.close()
    ...
    if 'user_id' in session:
        try:
            ...
            with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                g.user = cur.fetchone()
                # Принудительное назначение админа для учетной записи vest
                if g.user and g.user['username'] == 'vest' and not g.user['is_admin']:
                    db.cursor().execute("UPDATE users SET is_admin = TRUE WHERE id = %s", (g.user['id'],))
...
body{font-family:'Inter',sans-serif;background:#03030a;color:#f1f5f9;min-height:100vh;overflow-x:hidden;background-image: linear-gradient(rgba(5, 5, 15, 0.85), rgba(5, 5, 15, 0.85)), url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1920');background-size:cover;background-position:center;background-attachment:fixed;}
...
.sidebar{position:fixed;top:0;right:0;width:260px;height:100vh;background:rgba(7,7,18,0.96);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-left:1px solid rgba(255,255,255,0.05);z-index:1000;transition:transform 0.4s cubic-bezier(0.4, 0, 0.2, 1);padding:90px 20px 20px;display:flex;flex-direction:column;gap:8px;transform:translateX(100%)}
...
.bottom-sheet {position:fixed;bottom:0;left:0;right:0;background:#0d0d1f;border-top:1px solid rgba(255,255,255,0.08);border-radius:24px 24px 0 0;z-index:2010;transform:translateY(100%);transition:transform 0.35s cubic-bezier(0.4,0,0.2,1);max-height:75vh;display:flex;flex-direction:column}
...
input,textarea{width:100%;padding:14px;background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#f1f5f9;font-size:14px;outline:none;transition:0.2s}
input:focus,textarea:focus{border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,0.15)}

.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:18px}
.card{background:rgba(10,10,30,0.4);border:1px solid rgba(255,255,255,0.05);border-radius:20px;padding:22px;transition:0.3s cubic-bezier(0.4, 0, 0.2, 1);backdrop-filter:blur(15px);-webkit-backdrop-filter:blur(15px)}
.card:hover{border-color:rgba(99,102,241,0.3);transform:translateY(-4px);box-shadow:0 12px 30px rgba(0,0,0,0.4)}
...
.stat-box{text-align:center;padding:10px 6px;background:rgba(0,0,0,0.3);border-radius:12px;border:1px solid rgba(255,255,255,0.01)}
...
.filter-btn{background:rgba(99,102,241,0.1);border:1px solid rgba(99,102,241,0.2);color:#a5b4fc;padding:11px 24px;border-radius:14px;cursor:pointer;font-size:14px;font-weight:700}
.filter-drop{display:none;background:rgba(10,10,26,0.6);border:1px solid rgba(255,255,255,0.06);border-radius:20px;padding:24px;margin-top:14px;text-align:left;backdrop-filter:blur(20px)}
.filter-drop.show{display:block !important}

.form-box{max-width:440px;margin:40px auto;background:rgba(10,10,26,0.4);border:1px solid rgba(255,255,255,0.05);border-radius:24px;padding:32px;backdrop-filter:blur(20px)}
table{width:100%;border-collapse:separate;border-spacing:0 8px}
th{padding:10px 14px;text-align:left;color:#475569;font-size:11px;text-transform:uppercase}
td{padding:14px;background:rgba(255,255,255,0.01);border-top:1px solid rgba(255,255,255,0.02);border-bottom:1px solid rgba(255,255,255,0.02)}
td:first-child{border-radius:12px 0 0 12px}<td></td>
...
/* Секции вкладок в профиле */
.tab-section {display:none;animation:fadeIn 0.3s ease}
@keyframes fadeIn {from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)}}

.footer{text-align:center;padding:32px 20px;color:#475569;font-size:13px;border-top:1px solid rgba(255,255,255,0.03);margin-top:60px}
...
document.body.addEventListener('click', function(e) {
    ...
    // Динамическое переключение внутренних табов профиля
    var tabBtn = e.target.closest('.tab-trigger-btn');
    if (tabBtn) {
        var targetId = tabBtn.getAttribute('data-target');
        document.querySelectorAll('.tab-section').forEach(function(sec) { sec.style.display = 'none'; });
        document.querySelectorAll('.tab-trigger-btn').forEach(function(b) { b.classList.remove('btn-primary'); b.classList.add('btn-secondary'); });
...
def render_layout(title, content, show_nav=True):
    ...
    <div class="bottom-sheet" id="sheet_country">
        ...
        <div class="bottom-sheet-content">
            <input type="text" class="sheet-search-input" placeholder="Поиск страны..." style="margin-bottom:14px; background: rgba(0,0,0,0.4);">
...
def profile():
    ...
    if request.method == 'POST':
        ...
        if action == 'verify_phone':
            ...
            if result: 
                ...
                session['client_temp'] = temp_ss  # Сохраняем слепок сессии для предотвращения Code Expired!
    ...
    admin_btn = f'<a href="/admin" class="btn btn-secondary" style="width:100%;justify-content:center;margin-top:10px;border-color:rgba(165,180,252,0.3)">⚙️ Панель администратора</a>' if g.user["is_admin"] else ''

    content = f'''
    <div class="profile-section">
        <div class="profile-card" style="background: linear-gradient(135deg, rgba(255,255,255,0.02), rgba(255,255,255,0.00));">
            <div class="flex">
                <div class="avatar">U</div>
                <div style="flex:1">
                    <h3 style="font-size:20px; font-weight:800; color:#fff;">{g.user["username"]}</h3>
                    <span class="balance-badge" style="display:inline-block;margin-top:6px; background:rgba(99,102,241,0.12); border-color:rgba(99,102,241,0.2);">{g.user["balance"]:.2f} ₽</span>
                    <span style="margin-left:12px;color:#475569;font-size:13px; font-weight:600;">Продаж: <strong style="color:#a5b4fc;">{g.user.get("sales_count",0)}</strong></span>
                </div>
            </div>
            {admin_btn}
        </div>
        ...
        <div style="display:flex; gap:10px; margin-bottom:18px;">
            <button class="btn btn-secondary tab-trigger-btn" data-target="section-sell" style="flex:1; justify-content:center;">🚀 Продать аккаунт</button>
            <button class="btn btn-secondary tab-trigger-btn" data-target="section-transactions" style="flex:1; justify-content:center;">📊 Мои транзакции</button>
        </div>

        <!-- Скрытая вкладка продажи -->
        <div class="tab-section" id="section-sell">
            <div class="profile-card">
                <h3 style="font-size:16px; font-weight:800; margin-bottom:12px; color:#fff;">Добавление аккаунта</h3>
                <form method="POST">
                    ...
                    <input type="text" name="phone" placeholder="+79001234567" required style="background: rgba(0,0,0,0.25); margin-bottom:14px;">
                    <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;">Получить код подтверждения</button>
                </form>
            </div>
        </div>

        <!-- Скрытая вкладка транзакций -->
        <div class="tab-section" id="section-transactions">
            <div class="profile-card">
                <h3 style="margin-bottom:14px;font-size:16px; font-weight:800; color:#fff;">История баланса</h3>
...
def verify_code_page():
    ...
    if request.method == 'POST':
        ...
        if action == 'confirm_code':
            ...
            try:
                # Восстановление оригинального инстанса сессии
                ...
                try:
                    ...
                    session['has_2fa'] = False # Обычный аккаунт без 2FA
                    ...
                    session['has_2fa'] = True # Авто-определение наличия 2FA
