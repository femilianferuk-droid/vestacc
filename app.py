...
def init_db():
    ...
    with db.cursor() as cur:
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS withdrawals (id SERIAL PRIMARY KEY, user_id REFERENCES users(id), amount_rub DECIMAL(10,2), amount_usdt DECIMAL(10,6), address VARCHAR(200), status VARCHAR(20) DEFAULT 'pending', created_at TIMESTAMP DEFAULT NOW())")
...
body{font-family:'Inter',sans-serif;background:#030308;color:#f1f5f9;min-height:100vh;overflow-x:hidden;background-image:radial-gradient(circle at 80% 10%, rgba(139,92,246,0.06), transparent 500px), radial-gradient(circle at 20% 90%, rgba(59,130,246,0.03), transparent 500px);}
...
.logo{font-size:22px;font-weight:900;text-decoration:none;letter-spacing:-0.8px;background:linear-gradient(90deg, #6366f1, #3b82f6, #8b5cf6, #6366f1);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:shimmer(3s) linear infinite}

.balance-badge{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);padding:8px 18px;border-radius:30px;color:#a5b4fc;font-weight:700;font-size:14px}
.burger{width:36px;height:36px;background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.03);cursor:pointer;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:4px;border-radius:10px;z-index:1020;transition:0.2s}
.burger:hover{border-color:rgba(255,255,255,0.15);background:rgba(255,255,255,0.03)}
...
.sidebar a:hover{background:rgba(255,255,255,0.03);color:#fff;border-color:rgba(255,255,255,0.05)}
...
input,textarea,select{width:100%;padding:14px;background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.05);border-radius:14px;color:#f1f5f9;font-size:14px;outline:none;transition:0.2s;margin-bottom:14px}
input:focus,textarea:focus,select:focus{border-color:rgba(99,102,241,0.4);background:rgba(0,0,0,0.3)}

.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px}
.card{background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.03);border-radius:20px;padding:18px;transition:0.3s;backdrop-filter:blur(20px);display:flex;flex-direction:column;justify-content:space-between}
...
.card-title-link{font-weight:700;font-size:16px;text-decoration:none;color:#fff;transition:0.2s}
.card-title-link:hover{color:#a5b4fc}
.price-tag{background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);color:#34d399;padding:4px 12px;border-radius:30px;font-weight:800;font-size:14px}

.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:12px}
.stat-box{text-align:center;padding:8px 4px;background:rgba(0,0,0,0.2);border-radius:10px}
.stat-val{font-size:14px;font-weight:800;color:#818cf8}
.stat-lbl{font-size:10px;color:#475569;text-transform:uppercase;font-weight:700}

.tags{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:14px}
.tag{padding:3px 10px;border-radius:30px;font-size:10px;font-weight:700}
...
.card-seller{font-size:12px;color:#64748b;font-weight:500}
...
def account_detail(account_id):
    try:
        ...
        buy_btn = f'<form action="/buy/{a["id"]}" method="POST" style="flex:1;"><button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:12px;font-size:15px;">Купить</button></form>' if g.user and g.user['id'] != a['seller_id'] and not a['is_sold'] else ''
        ...
        <div style="display:flex; flex-direction:column; margin-bottom:14px;">
            ...
            <div class="spec-row-item"><span class="spec-lbl">Группы</span><span class="spec-val">{a["groups_count"]}</span></div>
        </div>
        {desc}</div></div>'''
