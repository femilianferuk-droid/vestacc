...
/* Анимированный фон для страниц входа */
.auth-bg {
    position: fixed;
    top: 0;
    left: 0;
...
.status-dot.online { background: #10b981; box-shadow: 0 0 8px rgba(16, 185, 129, 0.5); animation: pulse 2s infinite; }
...
.avatar-xl { width: 64px; height: 64px; background: linear-gradient(135deg, #6366f1, #8b5cf6); border-radius: 20px; display: flex; align-items: center; justify-content: center; font-size: 28px; font-weight: 800; color: #fff; }
.profile-info h3 { font-size: 24px; font-weight: 800; color: #fff; display: flex; align-items: center; gap: 10px; }
.profile-info .balance-large { font-size: 18px; font-weight: 700; color: #34d399; margin-top: 4px; }

.profile-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 28px; }
.profile-actions .btn { justify-content: center; padding: 14px; font-size: 15px; }

.tab-nav { display: flex; gap: 4px; background: #161622; padding: 4px; border-radius: 14px; border: 1px solid rgba(255,255,255,0.03); margin-bottom: 20px; }
...
.purchase-card, .my-acc-card { background: #161622; border: 1px solid rgba(255,255,255,0.03); border-radius: 18px; padding: 18px; margin-bottom: 12px; transition: border-color 0.3s; }
.purchase-card:hover, .my-acc-card:hover { border-color: rgba(255,255,255,0.08); }
...
def render_layout(title, content, show_nav=True, auth_mode=False):
    nav = navbar() if show_nav else '<div class="navbar"><a href="/" class="logo">Vest Accs</a></div>'
    auth_bg = '<div class="auth-bg"></div>' if auth_mode else ''
    sheets = f'''
    ...
    '''
    return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><title>{title}</title>{STYLE}</head><body>{auth_bg}{nav}<div class="main-content"><div class="container">{flash_msgs()} {content}</div></div>{sheets}{SCRIPT}</body></html>'''
...
def navbar():
    if g.user:
        ...
        '''
    return '<div class="navbar"><a href="/" class="logo">Vest Accs</a><div style="display:flex;gap:8px"><a href="/login" class="btn btn-outline btn-sm">Вход</a><a href="/register" class="btn btn-primary btn-sm">Регистрация</a></div></div>'
...
def index():
    try:
        ...
        cards = ''
        for a in accounts:
            rating_str, rating_count = get_seller_rating(a['seller_id'])
            rating_html = ''
            seller_badge = ''
            if rating_str is not None:
                rating_html = f'<span class="rating-badge" style="font-size:11px; padding:2px 6px;">★ {rating_str}</span>'
                seller_badge = '<span class="seller-badge"><span class="status-dot online"></span>Проверенный</span>'
            ...
            <div class="card">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:10px; border-top:1px solid rgba(255,255,255,0.04); padding-top:10px;">
                    <div class="card-seller">
                        <strong>{a["seller_name"]}</strong> {seller_badge} {rating_html}
                    </div>
...
def register():
    ...
    <div class="auth-container">
        <div class="auth-card">
            ...
            <form method="POST">
                ...
                <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:14px;font-size:15px;">Зарегистрироваться</button>
            </form>
    ...
    {footer()}'''
    return render_layout("Регистрация", content, show_nav=False, auth_mode=True)
...
def login():
    ...
    <div class="auth-container">
        <div class="auth-card">
            ...
            <p style="text-align:center;margin-top:20px;color:#64748b;font-size:13px;">Нет аккаунта? <a href="/register" style="color:#818cf8;font-weight:600;">Зарегистрироваться</a></p>
        </div>
    </div>
    {footer()}'''
    return render_layout("Вход", content, show_nav=False, auth_mode=True)
        ...
        for a in accounts:
            ...
            <div class="card">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:14px; border-top:1px solid rgba(255,255,255,0.03); padding-top:12px;">
                    <div class="card-seller">{a["seller_name"]} {rating_html}</div>
                    <a href="/account/{a["id"]}" class="btn btn-primary btn-sm">Купить</a>
                </div>
            </div>'''
        if not cards: cards = '<div class="empty"><h3>Ничего не найдено</h3></div>'
        content = f'''<h1 class="page-title">Результаты</h1><p class="page-sub">Найдено: {total}</p><div class="grid">{cards}</div>{pagination(page, total, "/filter?" + "&".join([f"{k}={v}" for k,v in request.args.items() if k != "page"]))}{footer()}'''
...
def profile():
    ...
    <div class="profile-section">
        <div class="profile-header">
            <div class="avatar-xl">V</div>
            <div class="profile-info">
                <h3>{g.user["username"]} {rating_html}</h3>
                <div class="balance-large">{g.user["balance"]:.2f} ₽</div>
            </div>
        </div>
        
        <div class="profile-actions">
            <button class="btn btn-primary window-trigger-btn" data-window="win-sell">Продать аккаунт</button>
            <a href="/deposit" class="btn btn-success">Пополнить</a>
            <button class="btn btn-secondary window-trigger-btn" data-window="win-withdraw">Вывести</button>
            <a href="/logout" class="btn btn-outline" style="grid-column: span 2;">Выйти из аккаунта</a>
        </div>
