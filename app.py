...
def index():
    try:
        ...
        for a in accounts:
            tags = ''.join([
                '<span class="tag tag-yellow">2FA</span>' if a['has_2fa'] else '',
                '<span class="tag tag-red">Спамблок</span>' if a['spamblock'] else '',
                '<span class="tag tag-purple">Premium</span>' if a['is_premium'] else '',
                f'<span class="tag tag-blue">{a["country"] or "?"}</span>',
                f'<span class="tag tag-green">{a["origin"] or "?"}</span>'
            ])
...
def deposit():
    if request.method == 'POST':
        ...
        if amount < 20: flash('Минимум 20 ₽', 'error')
            ...
            try:
                # Исправленный запрос к Crypto Bot API
                url = "https://pay.crypt.bot/api/createInvoice"
                headers = {"Content-Type": "application/json"}
                data = {
                    "asset": "USDT",
                    "amount": str(amount / 90),
                    "description": "Пополнение Vest Accs",
                    "allow_comments": False,
                    "allow_anonymous": False
                }
                r = requests.get(url, params={"api_key": CRYPTO_TOKEN, "asset": "USDT", "amount": str(amount / 90), "description": "Vest Accs"}, timeout=10)
                resp = r.json()
                if resp.get('ok'):
                    invoice_id = resp['result']['invoice_id']
                    pay_url = resp['result']['pay_url']
                    db = get_db()
                    with db.cursor() as cur: cur.execute("INSERT INTO crypto_invoices (user_id, invoice_id, amount_rub) VALUES (%s,%s,%s)", (g.user['id'], invoice_id, amount))
                    return redirect(pay_url)
                flash(f'Ошибка: {resp}', 'error')
            except Exception as e: flash(f'Ошибка создания счета: {e}', 'error')
