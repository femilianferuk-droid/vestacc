...
def init_db():
    ...
    with db.cursor() as cur:
        ...
        # Принудительные миграции структуры таблиц
        ...
        # Создание или обновление администратора vest:55337q
...
def get_seller_rating(seller_id):
    try:
        ...
        with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            # Находим продажи без отзывов, которые прошли более 3 дней назад (авто-отзывы 5 звезд)
            cur.execute("""
                SELECT p.id FROM purchases p 
                JOIN accounts a ON p.account_id = a.id 
                LEFT JOIN reviews r ON r.account_id = a.id
            ...
            if total_reviews == 0: return "Нет отзывов", 0
            return f"{total_stars / total_reviews:.1f} ★ ({total_reviews} отз.)", total_reviews
    except:
        return "5.0 ★", 0
...
body{font-family:'Inter',sans-serif;background:#05050c;color:#f1f5f9;min-height:100vh;overflow-x:hidden;background-image:radial-gradient(circle at top right, rgba(99,102,241,0.03), transparent 400px);}
.navbar{background:rgba(10,10,26,0.7);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-bottom:1px solid rgba(255,255,255,0.05);padding:14px 24px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:1010}
...
.balance-badge{background:linear-gradient(135deg, rgba(99,102,241,0.15), rgba(79,70,229,0.05));border:1px solid rgba(99,102,241,0.25);padding:8px 18px;border-radius:30px;color:#a5b4fc;font-weight:700;font-size:14px}
...
.burger span{display:block;width:18px;height:2px;background:#cbd5e1;border-radius:2px;transition:0.3s ease}
...
.sidebar{position:fixed;top:0;right:0;width:260px;height:100vh;background:rgba(7,7,18,0.98);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-left:1px solid rgba(255,255,255,0.05);z-index:1000;transition:transform 0.4s cubic-bezier(0.4, 0, 0.2, 1);padding:90px 20px 20px;display:flex;flex-direction:column;gap:8px;transform:translateX(100%)}
...
.sidebar a:hover{background:linear-gradient(90deg, rgba(99,102,241,0.1), transparent);color:#fff}

.overlay{position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(3,3,10,0.6);backdrop-filter:blur(6px);z-index:990;display:none}
.overlay.show{display:block !important}

.bottom-sheet-backdrop {position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.7);z-index:2000;display:none;backdrop-filter:blur(4px)}
.bottom-sheet-backdrop.active {display:block}
.bottom-sheet {position:fixed;bottom:0;left:0;right:0;background:#080814;border-top:1px solid rgba(255,255,255,0.08);border-radius:24px 24px 0 0;z-index:2010;transform:translateY(100%);transition:transform 0.35s cubic-bezier(0.4,0,0.2,1);max-height:75vh;display:flex;flex-direction:column}
.bottom-sheet.active {transform:translateY(0)}
.bottom-sheet-header {padding:18px 24px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(255,255,255,0.04)}
.bottom-sheet-header h3 {font-size:18px;font-weight:800;color:#fff}
.bottom-sheet-close {background:transparent;border:none;color:#64748b;font-size:24px;cursor:pointer}
...
.sheet-item {padding:12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);border-radius:12px;text-align:center;font-size:13px;font-weight:600;color:#cbd5e1;cursor:pointer;transition:0.2s}
.sheet-item:hover {background:rgba(99,102,241,0.08);border-color:rgba(99,102,241,0.2)}
.sheet-item.selected {background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;border-color:transparent}

.custom-select-trigger {width:100%;padding:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#cbd5e1;font-size:14px;font-weight:500;text-align:left;cursor:pointer;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center}
...
.page-sub{text-align:center;color:#64748b;margin-bottom:32px;font-size:15px}
...
.btn-secondary{background:rgba(255,255,255,0.02);color:#e2e8f0;border:1px solid rgba(255,255,255,0.06)}
...
input,textarea{width:100%;padding:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;color:#f1f5f9;font-size:14px;outline:none;transition:0.2s;margin-bottom:14px}
input:focus,textarea:focus{border-color:#6366f1}

.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:18px}
.card{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);border-radius:20px;padding:22px;transition:0.3s cubic-bezier(0.4, 0, 0.2, 1);backdrop-filter:blur(15px);-webkit-backdrop-filter:blur(15px)}
.card:hover{border-color:rgba(99,102,241,0.25);transform:translateY(-4px)}
...
.price-tag{background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.2);color:#34d399;padding:6px 14px;border-radius:30px;font-weight:800}

.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px}
.stat-box{text-align:center;padding:10px 6px;background:rgba(0,0,0,0.3);border-radius:12px}
...
.tag{padding:4px 10px;border-radius:30px;font-size:11px;font-weight:700}
...
.tag-purple{background:rgba(168,85,247,0.1);color:#c084fc;border:1px solid rgba(168,85,247,0.15)}

.filter-bar{text-align:center;margin-bottom:24px}
.filter-btn{background:rgba(99,102,241,0.06);border:1px solid rgba(99,102,241,0.15);color:#a5b4fc;padding:11px 24px;border-radius:14px;cursor:pointer;font-size:14px;font-weight:700;transition:0.2s}
.filter-btn:hover{background:rgba(99,102,241,0.15)}
.filter-drop{display:none;background:rgba(10,10,26,0.4);border:1px solid rgba(255,255,255,0.04);border-radius:20px;padding:24px;margin-top:14px;text-align:left;backdrop-filter:blur(20px)}
.filter-drop.show{display:block !important}

.form-box{max-width:440px;margin:40px auto;background:rgba(10,10,26,0.3);border:1px solid rgba(255,255,255,0.04);border-radius:24px;padding:32px;backdrop-filter:blur(20px)}
...
td:first-child{border-radius:12px 0 0 12px;border-left:1px solid rgba(255,255,255,0.02)}
td:last-child{border-radius:0 12px 12px 0;border-right:1px solid rgba(255,255,255,0.02)}

.profile-card{background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.04);border-radius:20px;padding:24px;margin-bottom:18px;backdrop-filter:blur(10px)}
.avatar{width:48px;height:44px;background:linear-gradient(135deg,#6366f1,#4f46e5);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:800}

/* Новые мини-окна для профиля */
.modal-window {position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(3,3,10,0.8);backdrop-filter:blur(10px);z-index:3000;display:none;align-items:center;justify-content:center}
.modal-window.active {display:flex}
.modal-window-content {background:#0a0a16;border:1px solid rgba(255,255,255,0.06);border-radius:24px;padding:28px;width:92%;max-width:550px;max-height:85vh;overflow-y:auto;position:relative}

.rating-badge {background:rgba(251,191,36,0.08);border:1px solid rgba(251,191,36,0.2);color:#fbbf24;padding:3px 9px;border-radius:30px;font-size:12px;font-weight:700;display:inline-flex;align-items:center;gap:4px}
</style>'''
...
document.body.addEventListener('click', function(e) {
    ...
    // Логика переключения мини-окон профиля
...
def index():
    try:
        ...
        for a in accounts:
            ...
            buy = f'<form action="/buy/{a["id"]}" method="POST" style="flex:1"><button type="submit" class="btn btn-primary btn-sm">Купить</button></form>' if g.user and g.user['id'] != a['seller_id'] else ''
            cards += f'<div class="card"><div class="card-row"><div><div class="card-title">{a["title"]}</div><div class="card-seller">{a["seller_name"]} <span class="rating-badge">★ {rating_str.split()[0]}</span></div></div><div class="price-tag">{a["price"]:.0f} ₽</div></div><div class="stats"><div class="stat-box"><div class="stat-val">{a["chats_count"]}</div><div class="stat-lbl">Чаты</div></div><div class="stat-box"><div class="stat-val">{a["channels_count"]}</div><div class="stat-lbl">Каналы</div></div><div class="stat-box"><div class="stat-val">{a["groups_count"]}</div><div class="stat-lbl">Группы</div></div></div><div class="tags">{tags}</div><div class="card-actions"><a href="/account/{a["id"]}" class="btn btn-secondary btn-sm" style="justify-content:center">Подробнее</a>{buy}</div></div>'
...
def filter_accounts():
    try:
        ...
        for a in accounts:
            rating_str, _ = get_seller_rating(a['seller_id'])
            cards += f'<div class="card"><div class="card-row"><div><div class="card-title">{a["title"]}</div><div class="card-seller">{a["seller_name"]} <span class="rating-badge">★ {rating_str.split()[0]}</span></div></div><div class="price-tag">{a["price"]:.0f} ₽</div></div><div class="card-actions"><a href="/account/{a["id"]}" class="btn btn-secondary btn-sm" style="justify-content:center">Подробнее</a>' + (f'<form action="/buy/{a["id"]}" method="POST" style="flex:1"><button type="submit" class="btn btn-primary btn-sm">Купить</button></form>' if g.user and g.user['id'] != a['seller_id'] else '') + '</div></div>'
...
def profile():
    ...
    hist_html = ''.join([f'<tr><td>{h["created_at"].strftime("%d.%m %H:%M")}</td><td style="color:{"#34d399" if h["amount"] > 0 else "#fca5a5"}">{"+" if h["amount"] > 0 else ""}{h["amount"]:.2f} ₽</td><td>{h["description"]}</td></tr>' for h in history]) or '<tr><td colspan="3" style="text-align:center;color:#64748b;">Нет операций</td></tr>'
    accs_html = ''.join([f'<tr><td>#{a["id"]}</td><td><strong>{a["title"]}</strong></td><td>{a["price"]:.0f} ₽</td><td style="color:{"#34d399" if not a["is_sold"] else "#ef4444"}">{"Активен" if not a["is_sold"] else "Продан"}</td></tr>' for a in my_accs]) or '<tr><td colspan="4" style="text-align:center;color:#64748b;">Вы еще не добавляли товары</td></tr>'
    
    admin_btn = f'<a href="/admin" class="btn btn-secondary" style="width:100%;justify-content:center;margin-top:12px;border-color:rgba(165,180,252,0.2)">⚙️ Панель администратора</a>' if g.user["is_admin"] else ''
    ...
    <div class="profile-section">
        <div class="profile-card">
            <div class="flex">
                ...
                <div style="flex:1">
                    <h3 style="font-size:20px; font-weight:800; color:#fff;">{g.user["username"]} <span class="rating-badge">★ {rating_str.split()[0]}</span></h3>
        ...
        <div style="display:flex; flex-direction:column; gap:10px;">
            <button class="btn btn-secondary window-trigger-btn" data-window="win-sell" style="justify-content:center; width:100%; padding:14px; font-size:15px;">🚀 Продать аккаунт</button>
            <button class="btn btn-secondary window-trigger-btn" data-window="win-transactions" style="justify-content:center; width:100%; padding:14px; font-size:15px;">📊 Мои транзакции</button>
            <button class="btn btn-secondary window-trigger-btn" data-window="win-accounts" style="justify-content:center; width:100%; padding:14px; font-size:15px;">🗂️ Мои товары (аккаунты)</button>
        </div>

        <div class="modal-window" id="win-sell">
            <div class="modal-window-content">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;"><h3>Добавление товара</h3><button class="bottom-sheet-close window-close">&times;</button></div>
                <form method="POST">
                    <input type="hidden" name="action" value="verify_phone">
                    <label style="display:block;margin-bottom:6px;color:#64748b;font-size:13px;font-weight:600;">Номер телефона аккаунта</label>
        ...
        <div class="modal-window" id="win-transactions">
            <div class="modal-window-content" style="max-width:600px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;"><h3>История транзакций</h3><button class="bottom-sheet-close window-close">&times;</button></div>
        ...
        <div class="modal-window" id="win-accounts">
            <div class="modal-window-content" style="max-width:650px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;"><h3>Мои выставленные аккаунты</h3><button class="bottom-sheet-close window-close">&times;</button></div>
            ...
            try:
                ...
                with db.cursor() as cur:
                    cur.execute("INSERT INTO accounts (seller_id,title,origin,description,price,session_string,country,has_2fa,spamblock,is_premium,chats_count,channels_count,groups_count) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                        (g.user['id'],title,origin,desc,price,ss,ad.get('country',''),ad.get('has_2fa',has_2fa),ad.get('spamblock',False),ad.get('is_premium',False),ad.get('chats_count',0),ad.get('channels_count',0),ad.get('groups_count',0)))
                db.commit()
                for k in ['phone_verified','session_string','verify_phone','code_hash','client_temp','2fa_needed','has_2fa']: session.pop(k, None)
