...
def init_db():
    ...
    with db.cursor() as cur:
        ...
        # Принудительные миграции структуры таблиц (Фикс отсутствующих колонок)
...
body{font-family:'Inter',sans-serif;background:#03030c;color:#f1f5f9;min-height:100vh;overflow-x:hidden;background-image:radial-gradient(circle at 80% 10%, rgba(99,102,241,0.06), transparent 500px), radial-gradient(circle at 10% 90%, rgba(16,185,129,0.02), transparent 500px);}
::-webkit-scrollbar{width:6px;height:6px}
::-webkit-scrollbar-track{background:transparent}
::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1);border-radius:10px}
::-webkit-scrollbar-thumb:hover{background:rgba(255,255,255,0.2)}

.navbar{background:rgba(6,6,14,0.6);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-bottom:1px solid rgba(255,255,255,0.04);padding:14px 24px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:1010}
.logo{font-size:22px;font-weight:900;text-decoration:none;color:#fff;letter-spacing:-0.5px;background:linear-gradient(135deg, #cbd5e1, #a5b4fc);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.balance-badge{background:linear-gradient(135deg, rgba(99,102,241,0.12), rgba(99,102,241,0.02));border:1px solid rgba(99,102,241,0.2);padding:8px 18px;border-radius:30px;color:#a5b4fc;font-weight:700;font-size:14px}

.burger{width:38px;height:36px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);cursor:pointer;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:4px;position:relative;border-radius:10px;transition:0.2s;z-index:1020}
.burger span{display:block;width:18px;height:2px;background:#94a3b8;border-radius:2px;transition:0.3s cubic-bezier(0.4,0,0.2,1)}
...
.sidebar{position:fixed;top:0;right:0;width:270px;height:100vh;background:rgba(4,4,10,0.94);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-left:1px solid rgba(255,255,255,0.04);z-index:1000;transition:transform 0.4s cubic-bezier(0.4, 0, 0.2, 1);padding:90px 20px 20px;display:flex;flex-direction:column;gap:6px;transform:translateX(100%)}
...
.sidebar a:hover{background:linear-gradient(90deg, rgba(99,102,241,0.08), transparent);color:#fff}
...
.page-sub{text-align:center;color:#475569;margin-bottom:32px;font-size:15px;font-weight:500}
...
input:focus,textarea:focus{border-color:rgba(99,102,241,0.4);background:rgba(0,0,0,0.3)}

.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:20px}
.card{background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.03);border-radius:24px;padding:24px;transition:0.3s cubic-bezier(0.4, 0, 0.2, 1);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px)}
.card:hover{border-color:rgba(99,102,241,0.2);transform:translateY(-3px);box-shadow:0 12px 30px rgba(0,0,0,0.5)}
...
.price-tag{background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);color:#34d399;padding:6px 14px;border-radius:30px;font-weight:800;font-size:15px}

.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px}
.stat-box{text-align:center;padding:12px 6px;background:rgba(0,0,0,0.2);border-radius:14px;border:1px solid rgba(255,255,255,0.01)}
.stat-val{font-size:16px;font-weight:800;color:#818cf8}
.stat-lbl{font-size:11px;color:#475569;text-transform:uppercase;font-weight:700;margin-top:2px}
...
.tag-purple{background:rgba(168,85,247,0.08);color:#c084fc;border:1px solid rgba(168,85,247,0.12)}

.filter-bar{text-align:center;margin-bottom:24px}
.filter-btn{background:rgba(99,102,241,0.04);border:1px solid rgba(99,102,241,0.12);color:#a5b4fc;padding:12px 28px;border-radius:14px;cursor:pointer;font-size:14px;font-weight:700;transition:0.2s}
.filter-btn:hover{background:rgba(99,102,241,0.12)}
.filter-drop{display:none;background:rgba(6,6,16,0.5);border:1px solid rgba(255,255,255,0.04);border-radius:24px;padding:24px;margin-top:14px;text-align:left;backdrop-filter:blur(20px)}
...
.profile-card{background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.03);border-radius:24px;padding:24px;margin-bottom:18px;backdrop-filter:blur(20px)}
...
/* Высокопроизводительные мини-окна (Анимация на уровне ядра) */
.modal-window {position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(2,2,6,0.6);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);z-index:3000;opacity:0;visibility:hidden;display:flex;align-items:center;justify-content:center;transition:opacity 0.3s cubic-bezier(0.4,0,0.2,1), visibility 0.3s}
.modal-window.active {opacity:1;visibility:visible}
.modal-window-content {background:#05050d;border:1px solid rgba(255,255,255,0.06);border-radius:26px;padding:32px;width:92%;max-width:560px;max-height:82vh;overflow-y:auto;transform:scale(0.92);transition:transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)}
...
document.body.addEventListener('click', function(e) {
    ...
    // Родное управление мини-окнами (Плавный вызов)
...
def filter_accounts():
    try:
        ...
        for a in accounts:
            rating_str, _ = get_seller_rating(a['seller_id'])
            cards += f'<div class="card"><div class="card-row"><div><div class="card-title">{a["title"]}</div><div class="card-seller">{a["seller_name"]} <span class="rating-badge">★ {rating_str}</span></div></div><div class="price-tag">{a["price"]:.0f} ₽</div></div><div class="card-actions"><a href="/account/{a["id"]}" class="btn btn-secondary btn-sm" style="justify-content:center">Подробнее</a>' + (f'<form action="/buy/{a["id"]}" method="POST" style="flex:1"><button type="submit" class="btn btn-primary btn-sm">Купить</button></form>' if g.user and g.user['id'] != a['seller_id'] else '') + '</div></div>'
...
def profile():
    ...
    <div class="profile-section">
        ...
        <div style="display:flex; flex-direction:column; gap:10px;">
            <button class="btn btn-secondary window-trigger-btn" data-window="win-sell" style="justify-content:center; width:100%; padding:14px; font-size:14px;">🚀 Продать аккаунт</button>
            <button class="btn btn-secondary window-trigger-btn" data-window="win-transactions" style="justify-content:center; width:100%; padding:14px; font-size:14px;">📊 Мои транзакции</button>
            <button class="btn btn-secondary window-trigger-btn" data-window="win-accounts" style="justify-content:center; width:100%; padding:14px; font-size:14px;">🗂️ Мои товары (аккаунты)</button>
        </div>
