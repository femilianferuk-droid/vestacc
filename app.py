...
def handle_exception(e):
    ...
    <body style="background: #0b1329; color: #ff5555; font-family: monospace; padding: 30px; line-height: 1.5;">
        <div style="max-w: 1000px; margin: 0 auto; background: #1c2541; padding: 25px; border-radius: 12px; border: 1px solid #ff3333;">
            <h2 style="margin-top: 0; color: #f8fafc;">⚠️ Внутренняя ошибка Vest Accs</h2>
...
# --- Потоковый изолятор асинхронности для Serverless платформ ---
    ...
    <div class="bg-[#1c2541] rounded-xl p-5 border border-slate-700 shadow-xl">
        ...
        <form method="GET" id="filters-menu" class="hidden mt-5 grid grid-cols-1 md:grid-cols-4 gap-4 pt-4 border-t border-slate-700/50">
            ...
            <div class="flex items-end gap-2">
                ...
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-4 py-2 rounded-lg text-sm h-[38px] transition-all cursor-pointer">Применить</button>
            </div>
        </form>
    </div>
...
def render_vest_template(template_name, **kwargs):
    templates = {"main": MAIN_PAGE, "profile": PROFILE_PAGE, "auth": AUTH_TEMPLATE}
    # Безопасное Serverless-слияние строк во избежание ошибки TemplateNotFound
    full_template = BASE_TEMPLATE.replace("", templates[template_name])
