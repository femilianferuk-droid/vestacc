...
# Автоматическое создание таблиц при инициализации контекста приложения
...
def run_async(coro):
    """Безопасный запуск асинхронных тасок внутри синхронных роутов Flask"""
        ...
        <div class="bg-[#1c2541] rounded-xl border border-slate-700 p-5 flex flex-col justify-between hover:scale-[1.02] transition-all shadow-lg">
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t border-slate-700/50">
                ...
                <button 
                    ...
                    data-origin="{{ acc.origin }}"
                    data-country="{{ acc.country }}"
                    data-2fa="{{ acc.has_2fa }}"
                    data-spam="{{ acc.has_spamblock }}"
                    data-price="{{ acc.price }}"
                    data-desc="{{ acc.description }}"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-300">
                    ...
                    <tbody class="divide-y divide-slate-800">
                        ...
                        {% else %}
                        <tr>
                            <td colspan="3" class="text-center py-6 text-slate-500">Покупки отсутствуют.</td>
                        </tr>
                        {% endfor %}
                    </tbody>
                </table>
...
<script>
    function getTelegramCode(purchaseId) {
        ...
        fetch('/get_code/' + purchaseId)
            ...
            .catch(err => {
                val.innerText = "Ошибка соединения.";
            });
    }
</script>
{% endblock %}
...
def buy_account(account_id):
    ...
    if seller:
        seller.balance += (acc.price * 0.95) # Удержание комиссии площадки 5%
