...
def init_db():
    # Отдельное подключение, независимое от контекста запроса Flask
...
# Запуск создания таблиц в базе данных при инициализации приложения в продакшене
try:
    init_db()
...
def filter_accounts():
    try:
        ...
        return f'<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Поиск</title>{STYLE}</head><body>{navbar()}{sell_modal() if g.user else ""}{withdraw_modal() if g.user else ""}<div class="container"><h1 class="page-title">Результаты</h1><p class="page-sub">Найдено: {total}</p><div class="grid">{cards}</div>{pagination(page, total, "/filter?" + "&".join([f"{k}={v}" for k,v in request.args.items() if k != "page"]))}</div>{SCRIPT}</body></html>'
...
def my_purchases():
    try:
        ...
        return f'<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Покупки</title>{STYLE}</head><body>{navbar()}{sell_modal() if g.user else ""}{withdraw_modal() if g.user else ""}<div class="container"><h2 style="font-size:24px;font-weight:800;margin-bottom:18px">Мои покупки</h2>{items}</div>{SCRIPT}</body></html>'
    except Exception as e: return f'<h1>Ошибка: {e}</h1>', 500
