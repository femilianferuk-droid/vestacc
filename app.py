...
.burger{width:32px;height:32px;background:transparent;border:none;cursor:pointer;display:flex;flex-direction:column;justify-content:center;gap:4px;padding:5px;z-index:300;position:relative}
...
.card{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:14px;padding:18px;transition:0.3s;position:relative}
...
.price-tag{background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.2);color:#34d399;padding:5px 12px;border-radius:18px;font-weight:700;font-size:14px;white-space:nowrap}
...
.selected-tag .remove{cursor:pointer;font-size:12px;line-height:1}
...
function copyText(t){navigator.clipboard.writeText(t).then(()=>{let e=document.createElement('div');e.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;padding:10px 20px;border-radius:30px;font-weight:600;z-index:999;box-shadow:0 8px 25px rgba(99,102,241,0.5)';e.textContent='Скопировано!';document.body.appendChild(e);setTimeout(()=>{e.style.opacity='0';e.style.transition='0.3s';setTimeout(()=>e.remove(),300)},2000)})}
...
def navbar():
    if g.user:
        ...
        <div class="sidebar" id="sidebar">
            ...
            {f'<div class="divider"></div><a href="/admin">Админ-панель</a>' if g.user["is_admin"] else ""}
        </div>'''
    return f'''<div class="navbar">
        <a href="/" class="logo">Vest Accs</a>
...
def withdraw_modal():
    if not g.user: return ''
    can_withdraw = g.user['balance'] >= 50 and g.user.get('sales_count', 0) >= 1
    content = ''
    if can_withdraw:
        content = f'''<form method="POST" action="/withdraw"><div class="form-group"><label>Сумма (₽)</label><input type="number" name="amount_rub" step="0.01" max="{g.user['balance']}" placeholder="Минимум 50 ₽" required></div><p style="color:#64748b;font-size:12px;margin-bottom:12px">Курс: 1 USDT = 90 ₽ | Вы получите ~<span id="usdtCalc">0.00</span> USDT</p><div class="form-group"><label>Адрес TON</label><input type="text" name="address" placeholder="UQ... или EQ..." required></div><button type="submit" class="btn btn-primary" style="width:100%;justify-content:center">Отправить заявку</button></form><script>document.querySelector('[name=amount_rub]').addEventListener('input',function(){{document.getElementById('usdtCalc').textContent=(this.value/90).toFixed(6)}})</script>'''
    else:
        content = '<p style="color:#fca5a5;text-align:center;font-size:13px">Необходимо: баланс от 50 ₽ и минимум 1 продажа</p>'
    return f'''<div class="modal" id="withdrawModal"><div class="modal-bg" onclick="closeModal('withdrawModal')"></div><div class="modal-content"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 style="font-size:18px;font-weight:700">Вывод средств</h3><button onclick="closeModal('withdrawModal')" style="background:transparent;border:none;color:#94a3b8;font-size:22px;cursor:pointer">&times;</button></div><p style="color:#64748b;margin-bottom:12px;font-size:13px">Баланс: <strong style="color:#34d399">{g.user['balance']:.2f} ₽</strong> | Продаж: <strong style="color:#a5b4fc">{g.user.get('sales_count',0)}</strong></p>{content}</div></div>'''
    ...
    try:
        ...
        for a in accounts:
            ...
            if g.user and g.user['id'] != a['seller_id']:
                buy = f'<form action="/buy/{a["id"]}" method="POST" style="flex:1"><button class="btn btn-primary btn-sm" style="width:100%;justify-content:center">Купить</button></form>'
        ...
        if not cards: cards = '<div class="empty"><div class="empty-icon">-</div><h3>Нет аккаунтов</h3><p style="font-size:13px">Станьте первым продавцом</p></div>'
                ...
                <div class="filter-bar">
                    ...
                    <div class="filter-drop" id="filterDrop">
                        <form action="/filter" method="GET">
                            ...
                            <div style="display:flex;gap:6px;justify-content:center"><button type="submit" class="btn btn-primary btn-sm">Применить</button><a href="/" class="btn btn-secondary btn-sm">Сбросить</a></div>
                        </form>
                    </div>
                </div>
...
def register():
    if request.method == 'POST':
        ...
        if not username or not password: flash('Заполните все поля', 'error')
        else:
            try:
                db = get_db()
                with db.cursor() as cur:
                ...
                db.rollback(); flash('Пользователь существует', 'error')
    return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Регистрация</title>{STYLE}</head><body><div class="navbar"><a href="/" class="logo">Vest Accs</a></div><div class="form-box"><h2>Регистрация</h2><p class="sub">Создайте аккаунт</p>{flash_msgs()}<form method="POST"><div class="form-group"><label>Логин</label><input type="text" name="username" required></div><div class="form-group"><label>Пароль</label><input type="password" name="password" required></div><button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:12px">Зарегистрироваться</button></form><p style="text-align:center;margin-top:14px;color:#64748b;font-size:13px">Есть аккаунт? <a href="/login" style="color:#818cf8">Войти</a></p></div></body></html>'''
        ...
        try:
            ...
            with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                if user and verify_password(password, user['password_hash']):
                    session['user_id'] = user['id']; session.permanent = True; return redirect(url_for('index'))
            flash('Неверный логин или пароль', 'error')
        except Exception as e: flash(f'Ошибка: {e}', 'error')
    return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Вход</title>{STYLE}</head><body><div class="navbar"><a href="/" class="logo">Vest Accs</a></div><div class="form-box"><h2>Вход</h2><p class="sub">Войдите в аккаунт</p>{flash_msgs()}<form method="POST"><div class="form-group"><label>Логин</label><input type="text" name="username" required></div><div class="form-group"><label>Пароль</label><input type="password" name="password" required></div><button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:12px">Войти</button></form><p style="text-align:center;margin-top:14px;color:#64748b;font-size:13px">Нет аккаунта? <a href="/register" style="color:#818cf8">Создать</a></p></div></body></html>'''
...
def deposit():
    if request.method == 'POST':
        ...
        if amount < 20: flash('Минимальная сумма пополнения: 20 ₽', 'error')
        else:
            try:
                r = requests.get(f"https://pay.crypt.bot/api/createInvoice?api_key={CRYPTO_TOKEN}&asset=USDT&amount={amount/90:.2f}&description=Пополнение Vest Accs&allow_comments=false&allow_anonymous=false", timeout=10)
                data = r.json()
                if data.get('ok'):
                    ...
                    db = get_db()
                    with db.cursor() as cur:
                        cur.execute("INSERT INTO crypto_invoices (user_id, invoice_id, amount_rub) VALUES (%s,%s,%s)", (g.user['id'], invoice_id, amount))
                    return redirect(pay_url)
                flash('Ошибка создания счета', 'error')
            except Exception as e: flash(f'Ошибка: {e}', 'error')
    return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Пополнение</title>{STYLE}</head><body><div class="navbar"><a href="/" class="logo">Vest Accs</a><span class="balance-badge">{g.user["balance"]:.2f} ₽</span></div><div class="form-box"><h2>Пополнение</h2><p class="sub">Баланс: <strong style="color:#34d399">{g.user["balance"]:.2f} ₽</strong></p>{flash_msgs()}<form method="POST"><div class="form-group"><label>Сумма (от 20 ₽)</label><input type="number" name="amount" step="0.01" min="20" required></div><button type="submit" class="btn btn-success" style="width:100%;justify-content:center;padding:12px">Пополнить через Crypto Bot</button></form></div></body></html>'''
...
def crypto_callback():
    try:
        ...
        if status == 'paid':
            db = get_db()
            with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute("SELECT * FROM crypto_invoices WHERE invoice_id = %s AND status = 'pending'", (invoice_id,))
                inv = cur.fetchone()
                if inv:
                    cur.execute("UPDATE users SET balance = balance + %s WHERE id = %s", (inv['amount_rub'], inv['user_id']))
                    cur.execute("INSERT INTO balance_history (user_id, amount, type, description) VALUES (%s,%s,%s,%s)", (inv['user_id'], inv['amount_rub'], 'deposit', 'Пополнение через Crypto Bot'))
                    cur.execute("UPDATE crypto_invoices SET status = 'paid' WHERE invoice_id = %s", (invoice_id,))
...
def filter_accounts():
    try:
        ...
        if countries:
            ...
            if clist:
                cconds = []; 
                for c in clist: cconds.append("a.country = %s"); params.append(c)
                conds.append(f"({' OR '.join(cconds)})")
        if origins:
            olist = [o.strip() for o in origins.split(',') if o.strip()]
            if olist:
                oconds = []
                for o in olist: oconds.append("a.origin = %s"); params.append(o)
        ...
        for a in accounts:
            ...
            if g.user and g.user['id'] != a['seller_id']:
                buy = f'<form action="/buy/{a["id"]}" method="POST" style="flex:1"><button class="btn btn-primary btn-sm" style="width:100%;justify-content:center">Купить</button></form>'
            cards += f'<div class="card"><div class="card-row"><div><div class="card-title">{a["title"]}</div><div class="card-seller">{a["seller_name"]}</div></div><div class="price-tag">{a["price"]:.0f} ₽</div></div><div class="card-actions"><a href="/account/{a["id"]}" class="btn btn-secondary btn-sm" style="justify-content:center">Подробнее</a>{buy}</div></div>'
        if not cards: cards = '<div class="empty"><div class="empty-icon">-</div><h3>Ничего не найдено</h3></div>'
        return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Поиск</title>{STYLE}</head><body>{navbar()}<div class="container"><h1 class="page-title">Результаты</h1><p class="page-sub">Найдено: {total}</p><div class="grid">{cards}</div>{pagination(page, total, "/filter?" + "&".join([f"{k}={v}" for k,v in request.args.items() if k != 'page']))}</div></body></html>'''
    except Exception as e: return f'<h1>Ошибка: {e}</h1>', 500

# Остальные маршруты без изменений: account_detail, buy_account, my_purchases, get_code, profile, sell_account, withdraw, admin_panel
...
def account_detail(account_id):
    try:
        ...
        if g.user and g.user['id'] != a['seller_id'] and not a['is_sold']:
            buy = f'<form action="/buy/{a["id"]}" method="POST"><button class="btn btn-primary" style="width:100%;justify-content:center;padding:12px">Купить аккаунт</button></form>'
    ...
    try:
        ...
        for p in purchases:
            ...
            if not p['code_retrieved']:
                cb = f'<button onclick="getCode({p["id"]})" class="btn btn-primary btn-sm" style="width:100%;justify-content:center">Получить код</button>'
        ...
        if not items: items = '<div class="empty"><div class="empty-icon">-</div><h3>Нет покупок</h3><a href="/" class="btn btn-primary btn-sm" style="margin-top:8px">К покупкам</a></div>'
        return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Покупки</title>{STYLE}</head><body>{navbar()}<div class="container"><h2 style="font-size:24px;font-weight:800;margin-bottom:18px">Мои покупки</h2>{items}</div>{SCRIPT}</body></html>'''
    except Exception as e: return f'<h1>Ошибка: {e}</h1>', 500
...
def profile():
    if request.method == 'POST':
        ...
        if action == 'verify_phone':
            ...
            try:
                ...
                except SessionPasswordNeededError: session['2fa_needed'] = True; session['client_temp'] = c.session.save(); flash('Нужен 2FA', 'info')
    ...
    hist_html = ''
    for h in history:
        color = '#34d399' if h['amount'] > 0 else '#fca5a5'; sign = '+' if h['amount'] > 0 else ''
        hist_html += f'<tr><td>{h["created_at"].strftime("%d.%m %H:%M")}</td><td style="color:{color}">{sign}{h["amount"]:.2f} ₽</td><td style="color:#94a3b8;font-size:12px">{h["description"]}</td></tr>'
    if not hist_html: hist_html = '<tr><td colspan="3" style="text-align:center;color:#64748b;font-size:13px">Нет операций</td></tr>'
    ...
    return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Профиль</title>{STYLE}</head><body>
        <div class="navbar"><a href="/" class="logo">Vest Accs</a><div style="display:flex;align-items:center;gap:10px"><span class="balance-badge">{g.user["balance"]:.0f} ₽</span><button class="burger" id="burger" onclick="toggleSidebar()"><span></span><span></span><span></span></button></div></div>
        <div class="overlay" id="overlay" onclick="closeSidebar()"></div>
        <div class="sidebar" id="sidebar"><a href="/profile">Профиль</a><a href="/my_purchases">Мои покупки</a><a href="#" onclick="openModal('sellModal');closeSidebar()">Выставить аккаунт</a><a href="#" onclick="openModal('withdrawModal');closeSidebar()">Вывод средств</a><div class="divider"></div><a href="/deposit">Пополнить баланс</a><a href="/logout" style="color:#fca5a5">Выйти</a>{f'<div class="divider"></div><a href="/admin">Админ-панель</a>' if g.user["is_admin"] else ""}</div>
        {sell_modal()}{withdraw_modal()}
        <div class="container profile-section">
            {flash_msgs()}
            <div class="profile-card"><div class="flex"><div class="avatar">U</div><div><h3 style="font-size:18px">{g.user["username"]}</h3><span class="balance-badge" style="display:inline-block;margin-top:4px">{g.user["balance"]:.2f} ₽</span><span style="margin-left:8px;color:#64748b;font-size:12px">Продаж: {g.user.get("sales_count",0)}</span></div></div></div>
            <div class="profile-card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><h3 style="font-size:16px">Продажа аккаунта</h3><button onclick="openModal('sellModal')" class="btn btn-primary btn-sm">Выставить</button></div>{extra if extra else '<p style="color:#64748b;font-size:12px">Нажмите "Выставить" чтобы начать</p>'}</div>
            <div class="profile-card"><h3 style="margin-bottom:14px;font-size:16px">История баланса</h3><div style="overflow-x:auto"><table><thead><tr><th>Дата</th><th>Сумма</th><th>Описание</th></tr></thead><tbody>{hist_html}</tbody></table></div></div>
        </div>{SCRIPT}</body></html>'''

def send_code(phone):
    c = None
    try: c = TelegramClient(StringSession(), API_ID, API_HASH); c.connect(); r = c.send_code_request(phone); return r.phone_code_hash
    except: return None
    finally:
        if c:
            try: c.disconnect()
...
def sell_account():
    ...
    if request.method == 'POST':
        ...
        if not title or not price: flash('Название и цена обязательны', 'error')
            ...
            try:
                ss = session.get('session_string'); flash('Сбор данных...', 'info'); ad = gather_data(ss)
                db = get_db()
                with db.cursor() as cur:
...
def withdraw():
    ...
    if amount_rub < 50: flash('Минимальная сумма вывода: 50 ₽', 'error'); return redirect(url_for('profile'))
...
def admin_panel():
    ...
    try:
        ...
        for w in withdrawals:
            ...
            btns = ''
            if w['status'] == 'pending':
                btns = f'<form method="POST" style="display:inline"><input type="hidden" name="withdrawal_id" value="{w["id"]}"><button type="submit" name="action" value="approve" class="btn btn-success btn-sm" style="margin-right:3px">OK</button><button type="submit" name="action" value="reject" class="btn btn-red btn-sm">X</button></form>'
        ...
        return f'''<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Админ</title>{STYLE}</head><body><div class="navbar"><a href="/" class="logo">Vest Accs</a><div style="display:flex;align-items:center;gap:10px"><span class="balance-badge">{g.user["balance"]:.0f} ₽</span><button class="burger" id="burger" onclick="toggleSidebar()"><span></span><span></span><span></span></button></div></div><div class="overlay" id="overlay" onclick="closeSidebar()"></div><div class="sidebar" id="sidebar"><a href="/">Главная</a><a href="/profile">Профиль</a><a href="/logout" style="color:#fca5a5">Выйти</a></div><div class="container"><h2 style="font-size:24px;font-weight:800;margin-bottom:18px">Админ-панель</h2>{flash_msgs()}<div class="profile-card"><h3 style="margin-bottom:14px;font-size:16px">Изменить баланс</h3><form method="POST"><div class="form-group"><label>Пользователь</label><select name="user_id" required><option value="">Выберите...</option>{opts}</select></div><div class="form-group"><label>Сумма</label><input type="number" name="amount" step="0.01" required></div><div style="display:flex;gap:6px"><button type="submit" name="balance_action" value="add" class="btn btn-success btn-sm" style="flex:1;justify-content:center">Добавить</button><button type="submit" name="balance_action" value="set" class="btn btn-sm" style="flex:1;justify-content:center;background:#f59e0b;color:#000">Установить</button></div></form></div><div class="profile-card" style="overflow-x:auto"><h3 style="margin-bottom:14px;font-size:16px">Пользователи</h3><table><thead><tr><th>ID</th><th>Логин</th><th>Баланс</th><th>Продаж</th><th>Админ</th></tr></thead><tbody>{rows}</tbody></table></div><div class="profile-card" style="overflow-x:auto"><h3 style="margin-bottom:14px;font-size:16px">Заявки на вывод</h3><table><thead><tr><th>ID</th><th>User</th><th>RUB</th><th>USDT</th><th>Адрес</th><th>Статус</th><th>Дата</th><th></th></tr></thead><tbody>{w_rows}</tbody></table></div></div>{SCRIPT}</body></html>'''
    except Exception as e: return f'<h1>Ошибка: {e}</h1>', 500
