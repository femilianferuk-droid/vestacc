...
.logo{font-size:22px;font-weight:900;text-decoration:none;letter-spacing:-0.8px;background:linear-gradient(90deg, #6366f1, #3b82f6, #4f46e5, #6366f1);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:shimmer 3s linear infinite}
...
.bottom-sheet {position:fixed;bottom:0;left:0;right:0;background:#060610;border-top:1px solid rgba(255,255,255,0.06);border-radius:26px 24px 0 0;z-index:2010;transform:translateY(100%);transition:transform(0.35s);max-height:75vh;display:flex;flex-direction:column}
...
.card{background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.03);border-radius:24px;padding:24px;transition:0.3s;backdrop-filter:blur(20px);display:flex;flex-direction:column;justify-content:space-between}
        ...
        for a in accounts:
            ...
            <div class="card">
                <div>
                    ...
                    <div class="stats">
                        <div class="stat-box"><div class="stat-val">{a["chats_count"]}</div><div class="stat-lbl">Чаты</div></div>
                        <div class="stat-box"><div class="stat-val">{a["channels_count"]}</div><div class="stat-lbl">Каналы</div></div>
                        <div class="stat-box"><div class="stat-val">{a["groups_count"]}</div><div class="stat-lbl">Группы</div></div>
                    </div>
...
def account_detail(account_id):
    try:
        ...
        buy_btn = f'<form action="/buy/{a["id"]}" method="POST" style="flex:1;"><button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:12px;font-size:15px;">Купить аккаунт</button></form>' if g.user and g.user['id'] != a['seller_id'] and not a['is_sold'] else ''
        check_btn = f'<button class="btn btn-secondary btn-check-valid" data-id="{a["id"]}" style="flex:1;justify-content:center;padding:12px;font-size:15px;">Проверить на валид</button>' if not a['is_sold'] else ''
        ...
        <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px">
            <div>
                ...
                <p style="color:#64748b;font-size:14px; margin-top:4px;">Продавец: <strong style="color:#f1f5f9;">{a["seller_name"]}</strong> <span class="rating-badge" style="margin-left:6px;">★ {rating_str}</span></p>
            </div>
            <div style="background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);padding:10px 20px;border-radius:16px;text-align:center">
                <div style="font-size:10px;color:#475569; text-transform:uppercase; font-weight:700;">Цена</div>
                <div style="font-size:22px;font-weight:900;color:#34d399; margin-top:2px;">{a["price"]:.2f} ₽</div>
            </div>
        </div>
