...
body{font-family:'Inter',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;overflow-x:hidden;display:flex;flex-direction:column;line-height:1.5}
...
.sidebar a:hover{background:rgba(42,171,238,0.06);color:#fff;border-color:var(--border-active)}
...
.btn-secondary:hover{background:rgba(255,255,255,0.04);color:#fff;border-color:var(--border-active)}
...
.acc-left{display:flex;align-items:center;gap:16px;flex-wrap:wrap;flex:1;min-width:0}
.acc-title{font-weight:600;font-size:14px;color:#fff;text-decoration:none;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.acc-seller{font-size:11px;color:var(--text-muted)}
...
.sheet-item:hover{background:rgba(42,171,238,0.06);color:#fff}
...
.modal{
    ...
    opacity:0;visibility:hidden;display:flex;align-items:center;justify-content:center;transition:0.3s
}
.modal.active{opacity:1;visibility:visible}
.modal-content{
    background:var(--bg-card);border:1px solid rgba(255,255,255,0.06);
    border-radius:var(--radius-lg);padding:28px;width:92%;max-width:520px;
    max-height:85vh;overflow-y:auto;transform:scale(0.95);transition:0.3s
}
...
.purchase-card{
    background:var(--bg-card);border:1px solid var(--border);
    border-radius:var(--radius-lg);padding:22px;margin-bottom:14px;animation:fadeInUp 0.3s ease
}
...
.filter-btn{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);color:#fff;width:100%;padding:13px;border-radius:var(--radius);cursor:pointer;font-weight:600;font-size:14px;font-family:inherit}
...
.pagination a:hover{background:rgba(42,171,238,0.08);color:#fff}
...
@media(max-width:768px){
    ...
    .account-card-compact{padding:12px 14px}
    .acc-left{gap:8px}
...
@media(max-width:480px){
    .account-card-compact{flex-direction:column;align-items:flex-start}
    .acc-right{width:100%;justify-content:space-between}
...
document.body.addEventListener('click', function(e) {
    ...
    if (e.target.closest('.window-close') || e.target.classList.contains('modal')) {
        document.querySelectorAll('.modal').forEach(function(el) { el.classList.remove('active'); });
        return;
    }
    ...
    if (star) {
        ...
        document.getElementById('review-rating-' + purchaseId).value = rating;
        return;
    }
});
...
def render_account_cards(accounts):
    ...
    for a in accounts:
        ...
        prem_class = "purple" if a["is_premium"] else ""
        cards += f'''
        <div class="account-card-compact">
            <div class="acc-left">
                ...
                <div class="acc-tags">
                    ...
                    <span class="acc-tag {prem_class}">{"Premium" if a["is_premium"] else "Обычный"}</span>
                </div>
                <span class="acc-seller">{a["seller_name"]} {rating_html}</span>
            </div>
            <div class="acc-right">
...
def my_purchases():
    try:
        ...
        for p in purchases:
            ...
            auth_key_short = (str(ss.auth_key)[:25] + '...') if ss.auth_key else '—'
            cb = f'<button class="btn btn-primary btn-get-code" data-id="{p["id"]}" style="width:100%">Получить код</button>' if not p['code_retrieved'] else ''
            ...
            <div class="purchase-card">
                ...
                <div class="purchase-info">
                    ...
                    <div class="info-block">
                        ...
                        <button class="btn-copy" data-copy="{ss.auth_key}" style="margin-top:8px">Копировать</button>
                    </div>
                </div>
            ...
            <div class="modal" id="win-review-{p["id"]}">
                <div class="modal-content">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3>Оставить отзыв</h3><button class="bottom-sheet-close window-close">&times;</button></div>
...
def profile():
    ...
    <div class="modal" id="win-stats">
        <div class="modal-content">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3>Детальная статистика</h3><button class="bottom-sheet-close window-close">&times;</button></div>
