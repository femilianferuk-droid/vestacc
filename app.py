...
# Полностью переписанный SCRIPT с использованием Event Delegation (без inlineonclick)
SCRIPT = '''
<script>
document.addEventListener('DOMContentLoaded', function() {
    document.body.addEventListener('click', function(e) {
        ...
        // Логика клика по затемнению (overlay)
        ...
        // Логика кнопки фильтра
        var filterBtn = e.target.closest('.filter-btn');
        if (filterBtn) {
            var fd = document.getElementById('filterDrop');
            if(fd) fd.classList.toggle('show');
            return;
        }
        
        // Закрытие мультиселектов при клике мимо
