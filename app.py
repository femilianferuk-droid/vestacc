...
body{font-family:'Inter',sans-serif;background:#030308;color:#f1f5f9;min-height:100vh;overflow-x:hidden;background-image:radial-gradient(circle at 80% 10%, rgba(99,102,241,0.05), transparent 500px);}
.navbar{background:rgba(6,6,12,0.6);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-bottom:1px solid rgba(255,255,255,0.04);padding:14px 24px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:1010}

@keyframes shimmer { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
.logo{font-size:22px;font-weight:900;text-decoration:none;letter-spacing:-0.8px;background:linear-gradient(90deg, #6366f1, #a5b4fc, #ec4899, #6366f1);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:shimmer 4s linear infinite}

.balance-badge{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);padding:8px 18px;border-radius:30px;color:#a5b4fc;font-weight:700;font-size:14px}
.burger{width:36px;height:36px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);cursor:pointer;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:4px;border-radius:10px;z-index:1020}
...
.sidebar{position:fixed;top:0;right:0;width:270px;height:100vh;background:rgba(4,4,10,0.96);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-left:1px solid rgba(255,255,255,0.04);z-index:1000;transition:transform 0.4s cubic-bezier(0.4, 0, 0.2, 1);padding:90px 20px 20px;display:flex;flex-direction:column;gap:6px;transform:translateX(100%)}
.sidebar.open{transform:translateX(0) !important}
.sidebar a{display:flex;align-items:center;gap:12px;padding:14px;color:#cbd5e1;text-decoration:none;border-radius:12px;font-weight:600;transition:0.2s;font-size:14px}
.sidebar a:hover{background:rgba(255,255,255,0.04);color:#fff}

.overlay{position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(2,2,5,0.5);backdrop-filter:blur(4px);z-index:990;display:none}
...
.bottom-sheet {position:fixed;bottom:0;left:0;right:0;background:#060610;border-top:1px solid rgba(255,255,255,0.06);border-radius:26px 24px 0 0;z-index:2010;transform:translateY(100%);transition:transform 0.35s cubic-bezier(0.4,0,0.2,1);max-height:75vh;display:flex;flex-direction:column}
...
.sheet-item {padding:12px;background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.04);border-radius:12px;text-align:center;font-size:13px;font-weight:600;color:#94a3b8;cursor:pointer;transition:0.2s}
...
.custom-select-trigger {width:100%;padding:14px;background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.05);border-radius:14px;color:#cbd5e1;font-size:14px;font-weight:500;text-align:left;cursor:pointer;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center}
...
.page-title{font-size:36px;font-weight:900;text-align:center;margin:24px 0 6px;letter-spacing:-0.8px;background:linear-gradient(135deg,#fff,#94a3b8);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.page-sub{text-align:center;color:#475569;margin-bottom:32px;font-size:15px}

.btn{padding:12px 24px;border:none;border-radius:14px;cursor:pointer;font-size:14px;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:8px;transition:0.25s cubic-bezier(0.4, 0, 0.2, 1);font-family:inherit;white-space:nowrap}
.btn-primary{background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;box-shadow:0 4px 20px rgba(99,102,241,0.15)}
...
input,textarea{width:100%;padding:14px;background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.05);border-radius:14px;color:#f1f5f9;font-size:14px;outline:none;transition:0.2s;margin-bottom:14px}

.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:20px}
.card{background:rgba(255,255,255,0.01);border:1px solid rgba(255,255,255,0.03);border-radius:24px;padding:24px;transition:0.3s;backdrop-filter:blur(20px)}
.card:hover{border-color:rgba(99,102,241,0.25);transform:translateY(-3px)}
.card-row{display:flex;justify-content:space-between;align-items:start;margin-bottom:16px}
.card-title-link{font-weight:700;font-size:18px;text-decoration:none;color:#fff;transition:0.2s}
.card-title-link:hover{color:#a5b4fc}
.price-tag{background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);color:#34d399;padding:6px 14px;border-radius:30px;font-weight:800}

.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px}
.stat-box{text-align:center;padding:12px 6px;background:rgba(0,0,0,0.2);border-radius:14px}
.stat-val{font-size:16px;font-weight:800;color:#818cf8}
.stat-lbl{font-size:11px;color:#475569;text-transform:uppercase;font-weight:700}

.tags{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:18px}
.tag{padding:4px 12px;border-radius:30px;font-size:11px;font-weight:700}
.tag-yellow{background:rgba(245,158,11,0.08);color:#fbbf24;border:1px solid rgba(245,158,11,0.12)}
.tag-blue{background:rgba(99,102,241,0.08);color:#a5b4fc;border:1px solid rgba(99,102,241,0.12)}
.tag-red{background:rgba(239,68,68,0.08);color:#fca5a5;border:1px solid rgba(239,68,68,0.12)}
.tag-green{background:rgba(16,185,129,0.08);color:#34d399;border:1px solid rgba(16,185,129,0.12)}

.filter-bar{text-align:center;margin-bottom:24px}
.filter-btn{background:rgba(99,102,241,0.04);border:1px solid rgba(99,102,241,0.12);color:#a5b4fc;padding:12px 28px;border-radius:14px;cursor:pointer;font-size:14px;font-weight:700}
.filter-drop{display:none;background:rgba(6,6,16,0.5);border:1px solid rgba(255,255,255,0.04);border-radius:24px;padding:24px;margin-top:14px;backdrop-filter:blur(20px)}
.filter-drop.show{display:block !important}

.form-box{max-width:440px;margin:40px auto;background:rgba(10,10,26,0.2);border:1px solid rgba(255,255,255,0.04);border-radius:24px;padding:32px;backdrop-filter:blur(20px)}
table{width:100%;border-collapse:separate;border-spacing:0 8px}
th{padding:10px 14px;text-align:left;color:#475569;font-size:11px;text-transform:uppercase}
td{padding:14px;background:rgba(255,255,255,0.01);border-top:1px solid rgba(255,255,255,0.02);border-bottom:1px solid rgba(255,255,255,0.02);font-size:14px}
td:first-child{border-radius:14px 0 0 14px;border-left:1px solid rgba(255,255,255,0.02)}
td:last-child{border-radius:0 14px 14px 0;border-right:1px solid rgba(255,255,255,0.02)}
...
/* Спецификация товара на одной строке */
...
.footer{text-align:center;padding:32px 20px;color:#334155;font-size:13px;border-top:1px solid rgba(255,255,255,0.02);margin-top:60px}
...
document.body.addEventListener('click', function(e) {
    ...
    // Асинхронная проверка на Валидность (Без перезагрузки)
        ...
        fetch('/check_valid/' + aid)
            ...
            .then(function(d) {
                if (d.valid) {
                    ...
                    checkBtn.className = 'btn btn-sm btn-red';
...
def index():
    try:
        ...
        for a in accounts:
            ...
            cards += f'<div class="card"><div class="card-row"><div><a href="/account/{a["id"]}" class="card-title-link">{a["title"]}</a><div class="card-seller">{a["seller_name"]} <span class="rating-badge">★ {rating_str}</span></div></div><div class="price-tag">{a["price"]:.0f} ₽</div></div><div class="stats"><div class="stat-box"><div class="stat-val">{a["chats_count"]}</div><div class="stat-lbl">Чаты</div></div><div class="stat-box"><div class="stat-val">{a["channels_count"]}</div><div class="stat-lbl">Каналы</div></div><div class="stat-box"><div class="stat-val">{a["groups_count"]}</div><div class="stat-lbl">Группы</div></div></div><div class="tags">{tags}</div><div class="card-actions" style="display:flex; gap:6px;">{buy}<button class="btn btn-secondary btn-sm btn-check-valid" data-id="{a["id"]}">Проверить</button></div></div>'
        if not cards: cards = '<div class="empty"><h3>Нет аккаунтов</h3></div>'
        
        content = f'''<h1 class="page-title">Маркетплейс Telegram</h1><p class="page-sub">Покупайте и продавайте аккаунты</p>
        <div class="filter-bar">
            <button class="filter-btn">Фильтры поиска</button>
            <div class="filter-drop" id="filterDrop">
                <form action="/filter" method="GET">
                    ...
                    <div style="display:flex;gap:6px;justify-content:center;margin-top:10px">
                        <button type="submit" class="btn btn-primary btn-sm" style="flex:1;justify-content:center">Применить</button>
                        <a href="/" class="btn btn-secondary btn-sm" style="flex:1;justify-content:center">Сбросить</a>
                    </div>
                </form>
            </div>
        </div>
...
def filter_accounts():
    try:
        ...
        for a in accounts:
            rating_str, _ = get_seller_rating(a['seller_id'])
            cards += f'<div class="card"><div class="card-row"><div><a href="/account/{a["id"]}" class="card-title-link">{a["title"]}</a><div class="card-seller">{a["seller_name"]} <span class="rating-badge">★ {rating_str}</span></div></div><div class="price-tag">{a["price"]:.0f} ₽</div></div><div class="card-actions" style="display:flex; gap:6px;">' + (f'<form action="/buy/{a["id"]}" method="POST" style="flex:1"><button type="submit" class="btn btn-primary btn-sm" style="width:100%; justify-content:center;">Купить</button></form>' if g.user and g.user['id'] != a['seller_id'] else '') + f'<button class="btn btn-secondary btn-sm btn-check-valid" data-id="{a["id"]}">Проверить</button></div></div>'
...
def account_detail(account_id):
    try:
        ...
        buy = f'<form action="/buy/{a["id"]}" method="POST"><button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:14px; font-size:15px;">Купить аккаунт</button></form>' if g.user and g.user['id'] != a['seller_id'] and not a['is_sold'] else ''
        desc = f'<div style="background:rgba(255,255,255,0.01); border:1px solid rgba(255,255,255,0.03); padding:16px; border-radius:16px; margin:16px 0"><div style="font-size:11px;color:#475569;text-transform:uppercase; font-weight:700; letter-spacing:0.5px;">Описание продавца</div><p style="margin-top:6px;font-size:14px; color:#cbd5e1;">{a["description"] or "Описание отсутствует."}</p></div>'
        rating_str, _ = get_seller_rating(a['seller_id'])
        
        content = f'''<div style="max-width:650px;margin:0 auto;"><div class="card" style="padding:24px"><div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px"><div><h2 style="font-size:24px;font-weight:800; color:#fff;">{a["title"]}</h2><p style="color:#64748b;font-size:14px; margin-top:4px;">Продавец: <strong style="color:#f1f5f9;">{a["seller_name"]}</strong> <span class="rating-badge" style="margin-left:6px;">★ {rating_str}</span></p></div><div style="background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);padding:10px 20px;border-radius:16px;text-align:center"><div style="font-size:10px;color:#475569; text-transform:uppercase; font-weight:700;">Цена товара</div><div style="font-size:22px;font-weight:900;color:#34d399; margin-top:2px;">{a["price"]:.2f} ₽</div></div></div>
        
        <div style="display:flex; flex-direction:column; margin-bottom:14px;">
            <div class="spec-row-item"><span class="spec-lbl">Страна (Гео)</span><span class="spec-val" style="color:#a5b4fc;">{a["country"] or "-"}</span></div>
            ...
            <div class="spec-row-item"><span class="spec-lbl">Двухфакторная защита (2FA)</span><span class="spec-val">{"Да" if a["has_2fa"] else "Нет"}</span></div>
            <div class="spec-row-item"><span class="spec-lbl">Наличие спамблока</span><span class="spec-val" style="color:{"#fca5a5" if a["spamblock"] else "#34d399"};">{"Есть" if a["spamblock"] else "Чистый"}</span></div>
            <div class="spec-row-item"><span class="spec-lbl">Telegram Premium</span><span class="spec-val" style="color:{"#c084fc" if a["is_premium"] else "#94a3b8"};">{"Активен" if a["is_premium"] else "Нет"}</span></div>
            <div class="spec-row-item"><span class="spec-lbl">Количество чатов</span><span class="spec-val">{a["chats_count"]}</span></div>
            <div class="spec-row-item"><span class="spec-lbl">Каналы</span><span class="spec-val">{a["channels_count"]}</span></div>
            <div class="spec-row-item"><span class="spec-lbl">Группы (Администрируемые)</span><span class="spec-val">{a["groups_count"]}</span></div>
        </div>
        {desc}{buy}</div></div>'''
...
def my_purchases():
    try:
        ...
        for p in purchases:
            ...
            items += f'''
            <div class="purchase-card" style="background:rgba(255,255,255,0.01); border:1px solid rgba(255,255,255,0.03); border-radius:20px; padding:20px; margin-bottom:12px;">
                <h3 style="margin-bottom:4px;font-size:17px; color:#fff;">{p["title"]}</h3>
                <p style="color:#475569;font-size:12px;margin-bottom:12px;">{p["purchase_date"].strftime("%d.%m.%Y %H:%M")}</p>
                <div style="background:rgba(0,0,0,0.2);padding:12px;border-radius:14px;margin-bottom:12px;font-size:14px; display:flex; align-items:center; justify-content:between;">
                    <span>Номер: <strong style="color:#fff;">{p["phone_number"]}</strong></span>
                    <button class="btn btn-secondary btn-sm btn-copy" data-text="{p["phone_number"]}" style="padding:4px 10px; font-size:11px; border-radius:8px;">Копировать</button>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:12px;">
                    <a href="/download_session/{p["id"]}" class="btn btn-secondary btn-sm" style="justify-content:center; font-size:12px;">💾 Скачать .session</a>
                    <a href="/download_json/{p["id"]}" class="btn btn-secondary btn-sm" style="justify-content:center; font-size:12px;">📄 Скачать JSON</a>
                </div>
                <div id="code-{p["id"]}"></div>{cb}
            </div>'''
...
def profile():
    ...
    admin_btn = f'<button class="btn btn-secondary window-trigger-btn" data-window="win-admin" style="justify-content:center; width:100%; border-color:rgba(165,180,252,0.15); color:#a5b4fc; margin-top:12px;">⚙️ Панель администратора</button>' if g.user["is_admin"] else ''
    ...
    <div class="profile-section">
        ...
        <div style="display:flex; flex-direction:column; gap:10px;">
            ...
            <button class="btn btn-secondary window-trigger-btn" data-window="win-accounts" style="justify-content:center; width:100%; padding:14px;">🗂️ Мои товары (аккаунты)</button>
        </div>
